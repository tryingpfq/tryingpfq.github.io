 ...
 * WorkerSender/WorkerReceiver   SendWorker/RecvWorker 把这两组线程搞懂，就能明白节点直接消息的通信
   ...
   **注意这里的通信，上面的通信过程是zk集群Leader选举之间的通信**
...
Leader节点运行后会周期性地向Follower发送心跳信息（称之为ping），如果一个Follower未收到Leader节点的心跳信息，
Follower节点的状态会从FOLLOWING转变为LOOKING。
每一个Follower的连接，Leader会创建一个LearnerHandler对象来处理与该Follower的消息通信
LearnerHandler创建一个独立线程，在主循环内不停地接受Follower的消息并根据消息类型决定如何处理
...
Leader发送NEWLEADER命令给Follower，该命令的作用应该是告诉Follower日志同步已经完成，Follower对该NEWLEADER作出ACK，
而Leader会等待该ACK消息；
Leader最后发送UPTODATE命令至Follower，这个命令的作用应该是告诉Follower，我已经收到了你的ACK，而Follower这边收到该消息
的时候说明一切与Leader同步的初始化工作都已经完成，可以进入正常的处理流程了，而Leader这边发完该命令后也可以进入正常的请求处理流程了。
 ...
 **总结上面几个通信过程**
 我发现，Leader选举是独立的通信Socket，节点之间的通信(心跳，同步，ACK等)是独立socket，然而和客户端的连接是基于NIOServer
